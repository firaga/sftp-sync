...
" Version:      1.1
" Edited by Jichen at 2014-11-20 12:07
...
" type=0 download
" type=1 update
function! s:makeCmdStr(type,host) 
  if exists('s:conf')
    let conf = s:conf
    unlet s:conf
  else
  ...
  let host=conf['host'].a:host
  let cmd = printf('expect -c "set timeout %d; spawn %s %s; while {1} { expect %s { send \"%s\r\"; exit; } %s timeout {exit;} eof {exit;} }"', timeout, sftp_cmd, host, cmpl_reg,exit_cmd,expect)
  return cmd
endfunction

function! SftpUpload(host)
  let cmd = s:makeCmdStr(1,a:host)
  if !empty(cmd)
    execute '!' . cmd
  endif
endfunction
...
function! SftpDownload(host)
  let cmd = s:makeCmdStr(0,a:host)
  if !empty(cmd)
    execute '!' . cmd
  endif
endfunction
...
nmap <leader>su :call SftpUpload(g:host1)<CR> <SPACE>
nmap <leader>sd :call SftpDownload(g:host1)<CR> <SPACE>
nmap <leader>du :call SftpUpload(g:host2)<CR> <SPACE>
nmap <leader>dd :call SftpDownload(g:host2)<CR> <SPACE>
